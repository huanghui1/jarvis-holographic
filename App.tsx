...
const MainApp: React.FC = () => {
  ...
  // Hover Tooltip State
  const [hoverState, setHoverState] = useState<{ label: string | null; x: number; y: number }>({ label: null, x: 0, y: 0 });
        ...
        {booted && !introActive && (
            <>
              <HandTrackingCanvas 
                  ...
                  hoveredLabel={hoverState.label}
                  hoverPosition={{ x: hoverState.x, y: hoverState.y }}
              />
              <HUDOverlay />
            </>
        )}
